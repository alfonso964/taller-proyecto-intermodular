...
function MisReparaciones() {
  ...
  const calcularEstadisticas = (datos) => {
    const stats = {
      total: datos.length,
      finalizadas: datos.filter(resultado => resultado.estado?.toUpperCase() === 'FINALIZADA').length,
      pendientes: datos.filter(resultado => resultado.estado?.toUpperCase() === 'PENDIENTE').length,
      vehiculos: new Set(datos.map(resultado => resultado.matricula?.toUpperCase() || `${resultado.marca}-${resultado.modelo}`)).size 
    };
  ...
  const reparacionesFiltradas = reparaciones.filter(respuesta => {
    const coincideEstado = filtroEstado === 'TODAS' || respuesta.estado?.toUpperCase() === filtroEstado;
    const textoBusqueda = busqueda.toLowerCase();
    const coincideTexto = 
      respuesta.marca?.toLowerCase().includes(textoBusqueda) ||
      respuesta.modelo?.toLowerCase().includes(textoBusqueda) ||
      respuesta.matricula?.toLowerCase().includes(textoBusqueda) ||
      respuesta.reparacion?.toLowerCase().includes(textoBusqueda);
